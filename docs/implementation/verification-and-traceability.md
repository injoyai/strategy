...
| SC-AC-11 | 页面层已覆盖：`ScreenerDetailPage.test.tsx` 的 `submits the frozen reference, not the editable draft`、`reports a preflight that does not pass as a refusal`、`keeps every input when a preflight is refused`（失败保留输入）、`reaches the preflight from the keyboard alone` 与 `submits the run from a form field with Enter`（键盘）；`ScreenRunPage.test.tsx` 的 `refuses to save an empty selection and says why it was empty`（空结果有原因）；断线：`src/api/sse.test.ts` 的 `clears an expired replay cursor before reconnecting after 410`、`App.test.tsx` 的 `shows a retryable service error when the API is unavailable`。**未覆盖**：真实浏览器端到端与窄屏渲染（断点已实现在 `styles.css` 的 1100/820/560，无浏览器验证与截图） |
