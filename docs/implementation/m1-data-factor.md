...
| M1-09 | 数据、标的池、因子页面闭环 | 后端 API 闭环（universes/factors/analyses + research 编排 + 错误码 + 幂等）与前端 `/universes`、`/factors`、`/factors/:id` 页面闭环（成员预览、因子预检/运行/分析、依赖回链、成功/失败/重试） | 仅界面跑通成功/失败/重试 |
