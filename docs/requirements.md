...
- 独立选股工作台：有类型条件筛选与因子输入、排序或加权评分、预检、不可变运行结果与逐条件解释、静态名单与导出。
...
| SCREEN-01 | 选股方案 | 保存不可变选股版本：输入绑定、条件树、排序/评分、数量与展示列；修改创建新版本 | P0 |
| SCREEN-02 | 条件筛选 | 有类型条件树支持比较、区间、集合与缺失检测，AND/OR/NOT 嵌套及三值逻辑 | P0 |
| SCREEN-03 | 因子选股 | 引用已注册因子版本与参数作为输入，复用 DataView 与因子计算，不另建公式 | P0 |
| SCREEN-04 | 排名与数量 | 多字段排序与加权评分互斥；all 或 top_n 数量选择；instrument_id 升序稳定决胜 | P0 |
| SCREEN-05 | 时点与预检 | 运行绑定快照、历史母池、决策时点与时区；预检返回依赖、PIT 与扫描量问题 | P0 |
| SCREEN-06 | 结果与解释 | 不可变名单、逐条件证据与互斥守恒汇总；发布前结果查询返回明确错误 | P0 |
| SCREEN-07 | 成果衔接 | 从成功运行保存带来源证据的静态标的池；CSV/JSON 导出 scope 明确 | P0 |
| SCREEN-08 | 界面与任务 | 选股运行复用 Job 的进度、取消、重试与恢复；全部操作页面完成 | P0 |
| REPLAY-01 | 历史会话 | 固定快照、历史股票母池、起止区间、时区、初始资金与模型版本；起点是首个供决策的历史日终时点 | P0 |
| REPLAY-02 | 逐日推进 | 手动前进一个交易日，停在确定的日终决策时点，不自动快进；起止日期含在复盘交易日列表内 | P0 |
| REPLAY-03 | 历史选股 | 使用当前会话 as_of 执行选股方案，查看排序、数据时间与入选原因；浏览器不能改为未来时点 | P0 |
| REPLAY-04 | 人工交易 | 用户明确选择方向、数量、订单类型、限价和有效期；支持撤销未成交数量 | P0 |
| REPLAY-05 | 成交模拟 | 市场资格、成交时序、费用、滑点、成交量约束、部分成交和拒单；后续事件才可能成交 | P0 |
| REPLAY-06 | 账户状态 | 现金、冻结资金、可用资金、持仓、可卖数量、成本、费用和盈亏 | P0 |
| REPLAY-07 | 操作证据 | 每次选股、下单、撤单、推进、成交与笔记形成可追溯日志 | P0 |
| REPLAY-08 | 会话恢复 | 离开页面后再打开可继续；服务重启不重复成交或丢失历史进度 | P0 |
| REPLAY-09 | 复盘报告 | 逐笔记录、净值/回撤、基准、持仓暴露、交易统计、导出 | P0 |
| REPLAY-10 | 界面闭环 | 可用鼠标和键盘完成全部操作，历史时间与当前现实时间清楚区分 | P0 |
...
| ScreenerVersion | 输入绑定、条件树、排序/评分、数量、展示列与规则 schema 版本 | 新增版本替代覆盖更新 |
...
| ScreenRun / ScreenResult | 冻结运行输入（方案、快照、母池、as_of）、引擎与政策版本、阶段汇总与解释 | 终态结果不可原地覆盖 |
...
| SC-AC-01 | SCREEN-02 | 数值/区间/集合与嵌套 AND/OR/NOT 的边界、类型及真值表符合定义；非法单位和操作符预检拒绝 |
| SC-AC-02 | SCREEN-02 | 空值不当 0；未知不因 NOT 变成 true；负盈利按明确条件处理 |
| SC-AC-03 | SCREEN-05 | 修改 as_of 之后的财报、行情、成分与状态，过去名单、排名、解释不变 |
| SC-AC-04 | SCREEN-04 | 多字段同值、评分分量同值、不同分片和分页下全局排名与 top_n 成员一致，instrument_id 决胜稳定 |
| SC-AC-05 | SCREEN-04 | 母体只有 1 个、全部同值、权重和非法、缺少评分值：百分位符合 0.5 规则；非法权重拒绝；缺分量不重新分权 |
| SC-AC-06 | SCREEN-04/06 | 0 个结果、不足 N、附加展示列缺失：正确成功空态/少量结果；不补入不合格标的、不因展示缺失任意排除 |
| SC-AC-07 | SCREEN-01 | 保存方案后换快照或日期再次运行：新 Run，旧配置/结果不可变；方案和静态名单语义清楚 |
| SC-AC-08 | SCREEN-08 | 并发提交、取消竞争、Worker 重启、旧租约发布：复用 Job 保证幂等和原子终态，半成品不作为成功结果 |
| SC-AC-09 | SCREEN-07 | 使用完整名单保存池、使用搜索过滤后的列表导出：scope 明确；不把当前页面当完整名单；导出与持久化结果一致 |
| SC-AC-10 | SCREEN-07 | t 时点名单用于 t 前的回测决策：拒绝并说明选择时点，不能回填历史；探索质量限制不丢失 |
| SC-AC-11 | SCREEN-08 | 页面成功、预检失败、断线、键盘、窄视口：无手写 JSON 也能完成闭环；失败保留配置、空结果有原因 |
| SC-AC-12 | SCREEN-06/08 | 切换工作区、越权解释/下载、过期游标：按统一鉴权与分页规则拒绝，不泄露其他结果 |
| RP-AC-01 | REPLAY-03 | 当前停在 D，查询 D+1 行情、财报、成分与导出：服务端拒绝未来读取，前端没有预加载未来数据 |
| RP-AC-02 | REPLAY-05 | 看完 D 日数据后下买单：当下无成交；推进后只在模型允许的后续事件成交 |
| RP-AC-03 | REPLAY-05 | 下一日跳空、限价未触及、停牌或零成交量：资金复核、未成交/拒单符合模型，不强制按昨收成交 |
| RP-AC-04 | REPLAY-05 | 多买单抢资金、多卖单抢库存、多单共享成交量：无透支/超卖/重复额度，执行次序固定 |
| RP-AC-05 | REPLAY-04/05 | 部分成交后撤单、DAY 过期、重复 Fill：保留已成交事实、释放余量预留，账本不重复写入 |
| RP-AC-06 | REPLAY-02 | 连点下一日、两标签页下单与推进竞争：幂等和 revision 保证仅一次推进/命令生效，冲突明确 |
| RP-AC-07 | REPLAY-08 | 日段提交前/后崩溃，旧 Worker 恢复或取消竞争：从准确检查点恢复，不丢成交、不重复发布、不回滚已提交日段 |
| RP-AC-08 | REPLAY-05/06 | 分红、拆股、退出母池、停牌缺价：持仓和现金正确，复权不双计，缺估值有显式状态 |
| RP-AC-09 | REPLAY-03/04 | 下单来自已过期筛选、未来筛选或其他会话：未来/跨会话引用拒绝；旧名单以当前市场和资金状态重新校验 |
| RP-AC-10 | REPLAY-09 | 到期仍持仓/有余单、最后时点下新单：结束取消余单并保留持仓，不用最后已知价伪造平仓 |
| RP-AC-11 | REPLAY-07 | 相同冻结输入和人工命令日志重放：名单、订单、成交、账本、检查点一致，指标符合政策容差 |
| RP-AC-12 | REPLAY-10/08 | 页面刷新、离开、断线、重连、空名单及无交易：可恢复同会话，历史时间清晰，空结果与未成交原因可理解 |
| RP-AC-13 | REPLAY-10 | 查别人会话的订单、解释或下载：工作区/会话权限一致，ID 不能绕过归属校验 |
| RP-AC-14 | REPLAY-09 | 完成复盘后重新练习同一区间：新会话留来源与尝试证据，旧结果不可改，不宣称未见样本 |
